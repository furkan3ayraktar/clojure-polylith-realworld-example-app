...
;; Look in:
;;   1. `core.cljs` for  "(dispatch-sync [web-ui/>initialise-db])"
;;   2. `events.cljs` for the registration of web-ui/>initialise-db handler
;;
...
;;
;; To see it used, look in `events.cljs` at the event handler for `web-ui/>initialise-db`.
...
(def >local-store-user ::local-store-user)
