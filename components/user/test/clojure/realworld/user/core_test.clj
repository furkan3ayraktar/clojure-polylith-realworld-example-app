...
(deftest login--user-not-found--return-negative-result
  (let [[ok? res] (core/login! {:email "test@example.com" :password "password"})]
    (is (false? ok?))
...
(deftest login--invalid-password--return-negative-result
  (let [_ (jdbc/insert! (test-db) :user {:email    "test@example.com"
        ...
        [ok? res] (core/login! {:email "test@example.com" :password "invalid-password"})]
    (is (false? ok?))
...
(deftest register!--user-exists-with-given-username--return-negative-result
  (let [_ (jdbc/insert! (test-db) :user {:username "username"})
        [ok? res] (core/register! {:email "test@example.com" :username "username"})]
    (is (false? ok?))
...
(deftest user-by-token--expired-token--return-negative-result
  (let [email        "test@example.com"
        username     "username"
        now          (zdt/now)
        ten-days-ago (zdt/minus-days now 10)
        token        (with-redefs [zdt/now (fn [] ten-days-ago)]
...
(deftest update-user!--user-exists-with-given-email--return-negative-result
  (let [_         (jdbc/insert! (test-db) :user {:email "test1@example.com"})
        auth-user (jdbc/insert! (test-db) :user {:email "test2@example.com"})
        [ok? res] (core/update-user! auth-user {:email "test1@example.com"})]
    (is (false? ok?))
    (is (= {:errors {:email ["A user exists with given email."]}} res))))
...
(deftest update-user!--user-exists-with-given-username--return-negative-result
  (let [_         (jdbc/insert! (test-db) :user {:username "username"})
        auth-user (jdbc/insert! (test-db) :user {:email "test2@example.com"})
        [ok? res] (core/update-user! auth-user {:email "test@example.com" :username "username"})]
    (is (false? ok?))
    (is (= {:errors {:username ["A user exists with given username."]}} res))))
